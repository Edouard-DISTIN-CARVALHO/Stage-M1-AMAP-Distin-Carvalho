...
dados_sum <- dados %>%  group_by(plot_code, date) %>%
  summarize(total_MgC_m2 = sum(total_litterfall_MgC_ha_year, na.rm = TRUE), 
            leaves = sum(leaves_MgC_ha_year, na.rm = TRUE), 
            twigs = sum(twigs_MgC_ha_year, na.rm = TRUE))
...
NLS_tot_an<-nls(data=dados_an, formula = 
     ...
     start=c(a = 365, b = .0001, c = 110, Base=20, p=2.5), control=list(maxiter=5000))
...
lines(x=ListDate,y=myFunc(x=as.numeric(ListDate-min(ListDate)),
                             ...
                             c = 120,
                             Base=20,
                             p=2.5),col="green", lty="dashed")

dados_an$day<-as.numeric(dados_an$date-min(dados_an$date,na.rm=TRUE))
NLS_lea_an<-nls(data=dados_an, formula = 
                  leaves~myFunc(x=day, a = a, b = b, c = c, Base=Base, p=p),
                start=c(a = 365, b = .0006, c = 120, Base=20, p=2.5), control=list(maxiter=5000))
